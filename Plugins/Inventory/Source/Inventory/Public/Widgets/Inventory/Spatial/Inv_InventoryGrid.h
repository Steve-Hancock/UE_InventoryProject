...
private:
	...
	void OnTileParametersUpdated(const FInv_TileParameters& NewTileParameters);
	...
	bool CursorExitedCanvas(const FVector2D& CanvasPosition, const FVector2D& BoundarySize, const FVector2D& MouseLocation);
	...
	UFUNCTION()
	void OnPopUpMenuConsumed(int32 Index);
	...
	// Index where an item would be placed if we click on a grid at a valid location.
